...
create table public.performance_evaluations (
  ...
  calibrated_score numeric(3, 2) null default null,
  ...
  new_supervisor_rating numeric null default null,
  new_calibrated_score numeric null default null,
  self_evaluation numeric null default null,
  goal_id integer null,
  constraint performance_evaluations_pkey primary key (id),
  constraint performance_evaluations_goal_id_fkey foreign KEY (goal_id) references performance_goals (id) on update CASCADE on delete CASCADE
...
create index IF not exists idx_perf_eval_goal_id on public.performance_evaluations using btree (goal_id) TABLESPACE pg_default;