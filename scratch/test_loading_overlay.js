...
async function main() {
    ...
    try {
        ...
        await send('Runtime.evaluate', {
            expression: `showStage1TableLoading(true, 'Updating Objectives...');`
        });
        await new Promise(r => setTimeout(r, 400));

        // Capture screenshot of the loading overlay
        const screenshot = await send('Page.captureScreenshot', { format: 'png' });
        fs.writeFileSync('scratch/table_loading.png', Buffer.from(screenshot.data, 'base64'));
        console.log('Loading screenshot saved to scratch/table_loading.png');
